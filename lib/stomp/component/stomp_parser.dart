...
class StompParser implements Parser {
  ...
  void parseData(dynamic data) {
    Uint8List byteList;
    if (data is String) {
      byteList = data.toUtf8Bytes;
    } else if (data is List<int>) {
      byteList = data.asUint8View;
    } else {
  ...
  String? _header(String name) {
    ...
    for (final e in _resultHeaders.entries) {
      if (e.key.toLowerCase() == n) return e.value;
    }
    return null;
  }

  void _setupCollectBody() {
    final cl = _header('content-length');
    if (cl != null) {
      final remaining = int.tryParse(cl);
      if (remaining == null) {
  ...
  void _consumeBody() {
    final type = _header(_CONTENT_TYPE_KEY)?.toLowerCase();

    if (type == _OCTET_STREAM_TYPE || type == null) {
      _binaryBody = _currentToken.asUint8View;
    } else {
  ...
  String _consumeTokenAsString() {
    final result = _currentToken.asUint8View.toUtf8String;
    _currentToken = [];
    return result;
  }
