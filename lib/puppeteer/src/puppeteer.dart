...
class Puppeteer {
  ...
  Future<Browser> launch({
    ...
    executablePath ??= await _inferExecutablePath(client);
  ...
  Future<Browser> connect({
    required HttpClientCore client,
    ...
    if (browserWsEndpoint != null) {
      ...
      var connectionURL = await _wsEndpoint(client, browserUrl);
...
Future<String> _wsEndpoint(HttpClientCore client, String browserURL) async {
  var response = await client.read(
    Uri.parse(p.url.join(browserURL, 'json/version')),
  );
...
Future<String> _inferExecutablePath(HttpClientCore client) async {
  ...
  if (executablePath != null) {
    ...
    return (await downloadChrome(client: client)).executablePath;
  }
}
