...
class IMfaSettings {
  ...
  IMfaSettings({required this.preferredMfa, required this.enabled});

  Map<String, dynamic> toMap() {
    return <String, dynamic>{'PreferredMfa': preferredMfa, 'Enabled': enabled};
  }
...
class CognitoUser {
  ...
  Future<CognitoUserSession?> _authenticateUserInternal(
    ...
    final deviceSecretVerifierConfig = {
      ...
      'PasswordVerifier': base64.encode(
        hex.decode(authenticationHelper.getVerifierDevices()!),
      ),
    };
    ...
    final dataConfirm = await client!.request(
      'ConfirmDevice',
      paramsConfirmDevice,
    );
  ...
  Future<CognitoUserSession?> refreshSession(
    ...
    final authParameters = {'REFRESH_TOKEN': refreshToken.getToken()};
  ...
  Future<CognitoUserSession?> getDeviceResponse() async {
    ...
    final authParameters = {'USERNAME': username, 'DEVICE_KEY': _deviceKey};
    ...
    final dataAuthenticate = await client!.request(
      'RespondToAuthChallenge',
  ...
  Future<CognitoUserSession?> _authenticateUserPlainUsernamePassword(
    ...
    final authResult = await client!.request(
      'InitiateAuth',
      await _analyticsMetadataParamsDecorator.call(paramsReq),
    );
  ...
  Future<CognitoUserSession?> _authenticateUserDefaultAuth(
    ...
    saltString = authenticationHelper.toUnsignedHex(
      challengeParameters['SALT'],
    );
    ...
    Future<dynamic> respondToAuthChallenge(challenge) async {
      ...
      try {
        dataChallenge = await client!.request(
          'RespondToAuthChallenge',
          challenge,
        );
      } on CognitoClientException catch (e) {
  ...
  dynamic resendConfirmationCode() async {
    final params = {'ClientId': pool.getClientId(), 'Username': username};
  ...
  Future<CognitoUserSession?> sendMFACode(
    ...
    try {
      dataAuthenticate = await client!.request(
        'RespondToAuthChallenge',
        await _analyticsMetadataParamsDecorator.call(paramsReq),
      );
    } on CognitoClientException catch (e) {
  ...
  Future forgotPassword() async {
    final paramsReq = {'ClientId': pool.getClientId(), 'Username': username};
    if (_clientSecretHash != null) {
      paramsReq['SecretHash'] = _clientSecretHash;
    }
    if (getUserContextData() != null) {
  ...
  Future<void> cacheTokens() async {
    ...
    await storage.setItem(
      idTokenKey,
      _signInUserSession?.getIdToken().getJwtToken(),
    );
    await storage.setItem(
      accessTokenKey,
      _signInUserSession?.getAccessToken().getJwtToken(),
    );
    await storage.setItem(
      refreshTokenKey,
      _signInUserSession?.getRefreshToken()?.getToken(),
    );
    await storage.setItem(
      clockDriftKey,
      '${_signInUserSession?.getClockDrift()}',
    );
  ...
  Future<List<CognitoUserAttribute>?> getUserAttributes() async {
    ...
    userData['UserAttributes'].forEach((attr) {
      attributeList.add(
        CognitoUserAttribute(name: attr['Name'], value: attr['Value']),
      );
    });
  ...
  Future<String?> getPreferredMFA() async {
    ...
    final userData = await client!.request('GetUser', {
      'AccessToken': _signInUserSession!.getAccessToken().getJwtToken(),
    });
